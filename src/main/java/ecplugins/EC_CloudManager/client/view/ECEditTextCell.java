 ...
 */
public class ECEditTextCell extends
    AbstractEditableCell<String, ECEditTextCell.ViewData> {

  interface MyTemplate extends SafeHtmlTemplates {
    @Template("<input type=\"text\" value=\"{0}\" size=\"{1}\" tabindex=\"-1\"></input>")
    SafeHtml input(String value,String size);
  }
  ...
  /**
   * Construct a new ECEditTextCell that will use a
   * {@link SimpleSafeHtmlRenderer}.
   */
  public ECEditTextCell() {
    this(SimpleSafeHtmlRenderer.getInstance());
  }

  public ECEditTextCell(String size) {
        this(SimpleSafeHtmlRenderer.getInstance());
        this.size = size;
  }
  
  /**
   * Construct a new ECEditTextCell that will use a given {@link SafeHtmlRenderer}
   * to render the value when not in edit mode.
   *
   * @param renderer a {@link SafeHtmlRenderer SafeHtmlRenderer<String>}
   *          instance
   */
  public ECEditTextCell(SafeHtmlRenderer<String> renderer) {
  ...
  public void render(Context context, String value, SafeHtmlBuilder sb) {
    ...
    if (viewData != null) {
      ...
      if (viewData.isEditing()) {
         ...
         */
        sb.append(template.input(text,size));
      } else {
